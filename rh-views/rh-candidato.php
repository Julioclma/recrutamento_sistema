...
include 'select-db/select_candidato.php';
$emailRecrutador = $_SESSION['email'];

include 'select-db/friends-selected.php';
...
<body>
    ...
    <div class="container-contents profile-candidato-info">
        ...
        if (!(empty($tel_number))) {
            echo ' <li class="list-group-item">Contato - ' . $tel_number . '</li></ul>
            <div class="card-body button-wpp">
            <a href="https://wa.me/+55' . $tel_number . '?text=Ol%C3%A1%2C+achei+voc%C3%AA+no+RH-Solid%C3%A1rio.+Prazer%2C+me+chamo+' . $userName . '" target="_blank" class="card-link">Chamar no Whatssap
            </a>
    ';
        }
        ...
        <form method="post" action="insert-db/friends-action.php">
        <input hidden="true" name="email_user_rh" value="' . $emailRecrutador . '">
        <input hidden="true" name="email_user_candidato" value="' . $email . '">
        <div class="card-body button-wpp"><button type="submit" class="btn btn-primary">+ Adicionar</div>
        </form></div>';
        }else{
            echo 'Pedido de amizade já enviado!</div>';
        }
